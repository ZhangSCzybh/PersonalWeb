...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| V1.0 | 2024 | 初始版本：首页、车辆管理、充电记录、数据分析、账单管理、资源导航 |
| V2.0 | 2024 | 新增衣橱管理模块：服饰上传管理、AI穿搭生成 |
...
| 衣橱管理 | /wardrobe | 服饰管理与AI穿搭 | admin | WardrobeTabs, ClothesGrid, UploadForm, AIOutfitGenerator |
...
### 5.8 衣橱分类表 (wardrobe_categories) - V2.0 新增
...
**索引：**
```sql
CREATE INDEX idx_wardrobe_categories_parent ON wardrobe_categories(parent_category);
```

---

### 5.9 服饰表 (clothes) - V2.0 新增
...
CREATE INDEX idx_clothes_favorite ON clothes(favorite_level DESC);
CREATE INDEX idx_clothes_seasons ON clothes(seasons);
```

---

### 5.10 穿搭记录表 (outfits) - V2.0 新增

| 字段 | 类型 | 必填 | 默认值 | 约束 | 描述 |
|------|------|------|--------|------|------|
| id | INTEGER | 是 | 自增 | PRIMARY KEY | 主键 |
| name | TEXT | 否 | - | - | 穿搭名称 |
| occasion | TEXT | 否 | - | - | 场合：日常/约会/通勤/正式/运动 |
| season | TEXT | 否 | - | - | 适用季节 |
...
### 5.11 穿搭服饰关联表 (outfit_items) - V2.0 新增
...
| item_type | TEXT | 是 | - | NOT NULL | 穿着位置：top/bottom/shoes/bag/accessory |
...
CREATE INDEX idx_outfit_items_clothes ON outfit_items(clothes_id);
```

---

### 5.12 数据模型 ER 图 (V2.0 更新)
